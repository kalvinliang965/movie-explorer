...
function MovieCard({ movie }: { movie: MovieCard }) {
	return (
		<div className="flex gap-3 p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
			{movie.posterPath ? (
				<img
...
export default function Movie(props: MovieProps) {
	...
	return (
		<div className="flex flex-col gap-3">
			...
			{movies.map((movie) => (
				<MovieCard key={movie.id} movie={toCard(movie)} />
			))}
		</div>
	);
}
